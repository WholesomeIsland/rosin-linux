...
use wayland_csd_frame::CursorIcon;
use wayland_csd_frame::{DecorationsFrame, FrameAction, FrameClick, ResizeEdge, WindowManagerCapabilities, WindowState};
...
const PRIMARY_COLOR_ACTIVE: u32 = 0xFF363636;
const PRIMARY_COLOR_INACTIVE: u32 = 0xFF111111;
...
pub struct FallbackFrame<State> {
    ...
    /// The subcompositor.
    subcompositor: Arc<WlSubcompositor>,
    compositor: Arc<WlCompositor>,
    ...
    State: Dispatch<WlSurface, ()> + Dispatch<WlSubsurface, ()> + 'static,
{
    ...
    fn supported_buttons(wm_capabilities: WindowManagerCapabilities) -> [Option<UIButton>; 3] {
        let maximize = wm_capabilities.contains(WindowManagerCapabilities::MAXIMIZE).then_some(UIButton::Maximize);
        let minimize = wm_capabilities.contains(WindowManagerCapabilities::MINIMIZE).then_some(UIButton::Minimize);
    ...
    fn draw_buttons(buttons: &[Option<UIButton>], canvas: &mut [u8], width: u32, scale: u32, is_active: bool, mouse_location: &Location) {
        let scale = scale as usize;
        for (idx, &button) in buttons.iter().flatten().enumerate() {
            if width >= (idx + 1) as u32 * HEADER_SIZE {
                if is_active && mouse_location == &Location::Button(button) {
                    Self::draw_button(canvas, idx * HEADER_SIZE as usize, scale, width as usize, BTN_HOVER_BG.to_le_bytes());
                }
                Self::draw_icon(canvas, width as usize, idx * HEADER_SIZE as usize, scale, BTN_ICON_COLOR.to_le_bytes(), button);
            }
        }
    }
    ...
    fn draw_button(canvas: &mut [u8], x_offset: usize, scale: usize, width: usize, btn_color: [u8; 4]) {
        ...
        // main square
        for y in 0..h * scale {
            let canvas = &mut canvas[(x_start + y * width) * 4 * scale..(x_start + y * width + h) * scale * 4];
        ...
        match icon {
            UIButton::Close => {
                ...
                for y in sh / 4..3 * sh / 4 {
                    let line = &mut canvas[(x_start + y * width + h / 4) * 4 * scale..(x_start + y * width + 3 * h / 4) * 4 * scale];
            ...
            UIButton::Maximize => {
                ...
                for y in 3 * sh / 8..5 * sh / 8 {
                    let line = &mut canvas[(x_start + y * width + 2 * h / 8) * 4 * scale..(x_start + y * width + 3 * h / 8) * 4 * scale];
                    for pixel in line.chunks_exact_mut(4) {
                        pixel[0] = icon_color[0];
                        pixel[1] = icon_color[1];
                        pixel[2] = icon_color[2];
                    ...
                    let line = &mut canvas[(x_start + y * width + 5 * h / 8) * 4 * scale..(x_start + y * width + 6 * h / 8) * 4 * scale];
                    for pixel in line.chunks_exact_mut(4) {
                        pixel[0] = icon_color[0];
                        pixel[1] = icon_color[1];
                        pixel[2] = icon_color[2];
    ...
    State: Dispatch<WlSurface, ()> + Dispatch<WlSubsurface, ()> + 'static,
{
    ...
    fn on_click(&mut self, _timestamp: Duration, click: FrameClick, pressed: bool) -> Option<FrameAction> {
        ...
        if click == FrameClick::Alternate {
            return if Location::Head != self.mouse_location || !self.wm_capabilities.contains(WindowManagerCapabilities::WINDOW_MENU) {
                None
            } else {
                Some(FrameAction::ShowMenu(self.mouse_coords.0, self.mouse_coords.1 - HEADER_SIZE as i32))
            };
        }
        ...
        match self.mouse_location {
            ...
            Location::Button(UIButton::Maximize) if !pressed && !self.state.contains(WindowState::MAXIMIZED) => Some(FrameAction::Maximize),
            Location::Button(UIButton::Maximize) if !pressed && self.state.contains(WindowState::MAXIMIZED) => Some(FrameAction::UnMaximize),
    ...
    fn click_point_moved(&mut self, _timestamp: Duration, surface_id: &ObjectId, x: f64, y: f64) -> Option<CursorIcon> {
        ...
        self.mouse_location = Self::precise_location(&self.buttons, location, self.render_data.as_ref().unwrap().parts[part_index].width, x, y);
        // Set dirty if we moved the cursor between the buttons.
        self.dirty |=
            (matches!(old_location, Location::Button(_)) || matches!(self.mouse_location, Location::Button(_))) && old_location != self.mouse_location;
    ...
    fn set_hidden(&mut self, hidden: bool) {
        ...
        if hidden {
            ...
            self.render_data = Some(FrameRenderData::new(&self.parent, &self.subcompositor, &self.compositor, &self.queue_handle));
        }
    }
    ...
    fn subtract_borders(&self, width: NonZeroU32, height: NonZeroU32) -> (Option<NonZeroU32>, Option<NonZeroU32>) {
        if self.state.contains(WindowState::FULLSCREEN) || self.render_data.is_none() {
            ...
            (NonZeroU32::new(width.get().saturating_sub(2 * BORDER_SIZE)), NonZeroU32::new(height.get().saturating_sub(HEADER_SIZE + 2 * BORDER_SIZE)))
        }
    }
    ...
    fn draw(&mut self) -> bool {
        ...
        let fill_color = if is_active { PRIMARY_COLOR_ACTIVE } else { PRIMARY_COLOR_INACTIVE }.to_le_bytes();
        ...
        for (idx, part) in render_data.parts.iter().enumerate() {
            ...
            let (buffer, canvas) =
                match self
                    .pool
                    .create_buffer(part.width as i32 * scale, part.height as i32 * scale, part.width as i32 * 4 * scale, wl_shm::Format::Argb8888)
                {
            ...
            if idx == HEADER {
                Self::draw_buttons(&self.buttons, canvas, part.width, scale as u32, is_active, &self.mouse_location);
            }
...
impl FrameRenderData {
    fn new<State>(parent: &WlSurface, subcompositor: &WlSubcompositor, compositor: &WlCompositor, queue_handle: &QueueHandle<State>) -> Self
    where
        State: Dispatch<WlSurface, ()> + Dispatch<WlSubsurface, ()> + 'static,
    {
        let parts = [
            // Header.
            FramePart::new(create_subsurface(parent.clone(), queue_handle, compositor, subcompositor), HEADER_SIZE, HEADER_SIZE, (0, -(HEADER_SIZE as i32))),
            // Top border.
            FramePart::new(
                create_subsurface(parent.clone(), queue_handle, compositor, subcompositor),
                BORDER_SIZE,
                BORDER_SIZE,
                (-(BORDER_SIZE as i32), -(HEADER_SIZE as i32 + BORDER_SIZE as i32)),
            ),
            // Right border.
            FramePart::new(create_subsurface(parent.clone(), queue_handle, compositor, subcompositor), BORDER_SIZE, BORDER_SIZE, (0, -(HEADER_SIZE as i32))),
            // Bottom border.
            FramePart::new(create_subsurface(parent.clone(), queue_handle, compositor, subcompositor), BORDER_SIZE, BORDER_SIZE, (-(BORDER_SIZE as i32), 0)),
            // Left border.
            FramePart::new(
                create_subsurface(parent.clone(), queue_handle, compositor, subcompositor),
                BORDER_SIZE,
